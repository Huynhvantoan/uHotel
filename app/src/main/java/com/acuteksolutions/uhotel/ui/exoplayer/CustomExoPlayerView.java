...
public class CustomExoPlayerView extends FrameLayout
        implements CustomPlaybackControlView.VisibilityListener, View.OnClickListener {
        ...
        if (attrs != null) {
            ...
            try {
                useController = a.getBoolean(
                        com.google.android.exoplayer2.R.styleable.SimpleExoPlayerView_use_controller,
                        useController);
                /* useTextureView = a.getBoolean(com.google.android.exoplayer2.R.styleable.SimpleExoPlayerView_use_texture_view,
                         useTextureView);*/
                resizeMode =
                        a.getInt(com.google.android.exoplayer2.R.styleable.SimpleExoPlayerView_resize_mode,
                                AspectRatioFrameLayout.RESIZE_MODE_FIT);
                rewindMs =
                        a.getInt(com.google.android.exoplayer2.R.styleable.SimpleExoPlayerView_rewind_increment,
                                rewindMs);
                fastForwardMs = a.getInt(
                        com.google.android.exoplayer2.R.styleable.SimpleExoPlayerView_fastforward_increment,
                        fastForwardMs);
                controllerShowTimeoutMs =
                        a.getInt(com.google.android.exoplayer2.R.styleable.SimpleExoPlayerView_show_timeout,
                                controllerShowTimeoutMs);
            } finally {
                a.recycle();
            }
    ...
    public void setControllerVisibilityListener(
            CustomPlaybackControlView.VisibilityListener listener) {
                        ...
                        if (Math.abs(y - currentY) > 300) {
                            if (y < currentY) {
                                ...
                                if (CurVolume < MaxVolume) {
                                    AudioUtil.getInstance(getContext())
                                            .setMediaVolume((CurVolume + 1) > MaxVolume ? MaxVolume : CurVolume + 1);
                                }
                                Logger.d("CurVolume " + CurVolume + " MaxV" + MaxVolume);
                            } else {
                                int CurVolume = AudioUtil.getInstance(getContext()).getMediaVolume();
                                int MaxVolume = AudioUtil.getInstance(getContext()).getMediaMaxVolume();
                                if (CurVolume > 0) {
                                    AudioUtil.getInstance(getContext())
                                            .setMediaVolume((CurVolume - 1) > 0 ? CurVolume - 1 : 0);
                                }
                                Logger.d("CurVolume " + CurVolume);
                            }
                        }
                    }
                    break;
...
/*    @Override
    ...
    public void setMoviesTitle(String title) {
        if (controller != null && !Preconditions.isEmpty(title))
            controller.setMoviesTitle(title);
    }

    public void setMoviesChannel(String channel) {
        if (controller != null && !Preconditions.isEmpty(channel))
            controller.setMoviesChannel(channel);
    }
}
